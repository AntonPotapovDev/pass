...
pub fn find_collisions<'a>(f: &'a PassListModel, s: &PassListModel) -> Vec<&'a String> {
    f.iter()
        .filter(|(key, value)| match s.get(*key) {
            Some(v) => v != *value,
            None => false,
        })
        .map(|(key, _value)| key)
...
pub fn safe_merge<'a>(f: &'a PassListModel, s: &mut PassListModel) -> Result<(), Vec<&'a String>> {
